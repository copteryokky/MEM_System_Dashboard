...
SESSION_FILE = DATA_DIR / "_active_session.txt"
...
# ---- Restore login จากไฟล์ session (กัน F5 / ปิดหน้าเบราว์เซอร์แล้วยังจำได้) ----
if not st.session_state.logged_in:
    username_from_file = None
    try:
        if SESSION_FILE.exists():
            username_from_file = SESSION_FILE.read_text(encoding="utf-8").strip()
    except Exception:
        username_from_file = None

    if username_from_file:
        display_name = get_user_display_name(username_from_file)
        if display_name:
            st.session_state.logged_in = True
            st.session_state.username = username_from_file
            st.session_state.display_name = display_name
            role = get_user_role(username_from_file) or "user"
            st.session_state.role = role
            if "current_menu" not in st.session_state or st.session_state.current_menu is None:
                st.session_state.current_menu = "หน้าหลัก" if role == "admin" else "รายการครุภัณฑ์"
            st.session_state.view = "app"
